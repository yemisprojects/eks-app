...
- [Maven](https://maven.apache.org) is a build automation tool used primarily for building an managing Java projects
...
- [Trivy](https://aquasecurity.github.io/trivy/v0.47/) is an open-source comprehensive vulnerability Scanner for containers and other artefacts. It is quick and canscan filesystems, images, kubernetes cluster, IaC configuration, Git repository making it one of the most versatile security scanners available.
...
1. When a user commits code to a Git repository, a webhook triggers the Jenkins pipeline
2. Maven builds and packages the application. This package can be uploaded to an artificat store such as Nexus.
3. Various tests are carried out such as unit and Code coverage test. It is also tested against coding standards using checkstyle. Results are analysed at a later step using SonarQube. 
...
7. If it passes, a docker image is built and scanned for vulnerabilities using Trivy. If the scan passes, the image is uploaded to docker hub. 
